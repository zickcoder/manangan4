...
export function CemeteryModule() {
  ...
  const filteredBurials = burials.filter(b =>
    ...
    b.permit_no.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-extrabold text-[#0f172a] font-display flex items-center gap-2.5">
            ...
            <span>Municipal Cemetery & Burial Management</span>
          </h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            size="sm"
            variant="outline"
            leftIcon={<PlusCircle className="w-4 h-4 text-purple-600" />}
            onClick={() => setIsBatchPlotsOpen(true)}
          >
            Add New Slots / Expansion
          </Button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        >
          <Card className="p-4 border-[#cbd5e1] h-full cursor-pointer hover:border-purple-300">
            <span className="text-[11px] font-bold text-slate-500 uppercase">Total Managed Slots</span>
            <p className="text-2xl font-extrabold text-slate-900 font-display mt-1">{totalPlots}</p>
            <span className="text-[10px] text-purple-600 font-semibold">Click to view all slots →</span>
          </Card>
        </button>
        ...
        >
          <Card className="p-4 border-[#cbd5e1] h-full cursor-pointer hover:border-emerald-300">
            <span className="text-[11px] font-bold text-emerald-700 uppercase">Available Plots</span>
            <p className="text-2xl font-extrabold text-emerald-600 font-display mt-1">{availableCount}</p>
            <span className="text-[10px] text-emerald-600 font-semibold">Click to filter Available →</span>
          </Card>
        </button>
        ...
        >
          <Card className="p-4 border-[#cbd5e1] h-full cursor-pointer hover:border-amber-300">
            <span className="text-[11px] font-bold text-amber-700 uppercase">Reserved Plots</span>
            <p className="text-2xl font-extrabold text-amber-600 font-display mt-1">{reservedCount}</p>
            <span className="text-[10px] text-amber-600 font-semibold">Click to filter Reserved →</span>
          </Card>
        </button>
        ...
        >
          <Card className="p-4 border-[#cbd5e1] h-full cursor-pointer hover:border-purple-400">
            <span className="text-[11px] font-bold text-purple-700 uppercase">Occupied Plots</span>
            <p className="text-2xl font-extrabold text-purple-700 font-display mt-1">{occupiedCount}</p>
            <span className="text-[10px] text-purple-700 font-semibold">Click to filter Occupied →</span>
          </Card>
        </button>
      </div>

      {/* How It Works Guide Banner */}
      <div className="p-4 bg-purple-50/70 rounded-2xl border border-purple-200 text-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-purple-950">
        <div className="flex items-center gap-2">
          <span className="font-bold">ℹ️ Plot Lifecycle Guide:</span>
          <span>
            <strong>Available</strong> ➔ <strong>Reserved</strong> (when citizen applies) ➔ <strong>Occupied</strong> (after funeral / interment is completed). Click any slot below to manage.
          </span>
        </div>
      </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
              activeSubTab === 'plots' ? 'bg-purple-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
            Interactive Plot & Columbarium Map ({plots.length})
          </button>
          <button
            onClick={() => setActiveSubTab('burials')}
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
              activeSubTab === 'burials' ? 'bg-purple-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
            Deceased Registry & Permits ({burials.length})
          </button>
        </div>

        {/* Cemetery Selector */}
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-slate-500">Cemetery:</span>
          <select
          ...
          >
            <option value="all">All Municipal Cemeteries</option>
      ...
      {/* =========================================================================
          SUB-TAB 1: INTERACTIVE PLOT MAP GRID (Click to Manage Status)
         ========================================================================= */}
      {activeSubTab === 'plots' && (
        <Card className="border-[#cbd5e1] p-6 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <CardTitle>Columbarium Wall Niches & Lawn Lot Layout</CardTitle>
              <CardDescription>Click any vault/plot below to view deceased details or update status.</CardDescription>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 pt-1">
            ...
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search by deceased name, vault code, lot no., section..."
                value={plotSearch}
          ...
          {(() => {
            ...
            return (
              <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2.5 max-h-[520px] overflow-y-auto pr-1">
                  {filtered.length === 0 ? (
                    <div className="col-span-full text-center py-12 text-slate-400">
                      <Search className="w-8 h-8 mx-auto mb-2 opacity-30" />
                      <p className="text-sm font-semibold">No slots match your search</p>
                      <p className="text-xs mt-1">Try a different name, code, or filter</p>
                    </div>
                  ) : (
                    filtered.map((plot) => (
                      <div
                        ...
                        className={`p-3 rounded-xl border transition-all cursor-pointer ${
                          plot.status === 'Available'
                            ? 'bg-emerald-50/70 border-emerald-300 hover:border-emerald-500 hover:shadow-soft'
                            : plot.status === 'Reserved'
                            ? 'bg-amber-50/70 border-amber-300 hover:border-amber-500 hover:shadow-soft'
                            : 'bg-slate-100 border-slate-300 hover:border-slate-400 opacity-90'
                        }`}
                        title={`Click to manage: ${plot.plot_code}`}
                      >
                        <div className="flex justify-between items-center">
                          <span className="font-mono text-[11px] font-extrabold text-slate-800 truncate">{plot.plot_code}</span>
                        ...
                        <div className="flex justify-between items-center mt-1 pt-1 border-t border-slate-100 text-[9px]">
      ...
      {/* =========================================================================
          SUB-TAB 2: DECEASED REGISTRY & PERMITS
         ========================================================================= */}
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider border-b border-slate-200">
                  <tr>
                    ...
                    <th className="py-3 px-4">Name of Deceased</th>
                    <th className="py-3 px-4">Date of Death</th>
                    <th className="py-3 px-4">Interment Date</th>
                    <th className="py-3 px-4">Assigned Plot</th>
                    <th className="py-3 px-4">Next of Kin</th>
                    <th className="py-3 px-4 text-right">Interment Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 font-medium">
                  {filteredBurials.map((b) => (
                    <tr key={b.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="py-3.5 px-4 font-mono font-bold text-purple-700">{b.permit_no}</td>
        ...
        {selectedPlotToManage && (
          <div className="space-y-4 text-xs">
            ...
            <div className="pt-3 flex justify-end gap-2 border-t border-slate-100">
              <Button size="sm" variant="outline" onClick={() => setIsManagePlotOpen(false)}>
                Cancel
              </Button>
              <Button size="sm" variant="primary" className="bg-purple-600 hover:bg-purple-700 font-bold" onClick={handleSavePlotStatus}>
                Update Plot Status
              </Button>
            </div>
          </div>
      ...
      <Modal
        ...
        title="Add Cemetery Plots / Expansion Wing"
        description="Add multiple burial slots or niches to the municipal database."
      >
        <form onSubmit={handleCreateBatchPlots} className="space-y-3 text-xs">
          <div>
          ...
          <div className="pt-3 flex justify-end gap-2 border-t border-slate-100">
            <Button size="sm" variant="outline" type="button" onClick={() => setIsBatchPlotsOpen(false)}>
              Cancel
            </Button>
            <Button size="sm" variant="primary" className="bg-purple-600 hover:bg-purple-700 font-bold" type="submit">
      ...
      >
        <form onSubmit={handleCreateBurial} className="space-y-3 text-xs">
          <Input
          ...
          <div>
            <label className="block text-xs font-semibold text-[#334155] mb-1">Select Cemetery Plot / Niche *</label>
            <select
          ...
          <div className="pt-3 flex justify-end gap-2">
            <Button size="sm" variant="outline" type="button" onClick={() => setIsNewBurialOpen(false)}>
              Cancel
            </Button>
            <Button size="sm" variant="primary" className="bg-purple-600 hover:bg-purple-700" type="submit">
      ...
      >
        {selectedBurial && (
          <div className="p-6 bg-white border-2 border-slate-800 rounded-2xl space-y-5 text-center text-xs">
            <div className="border-b border-slate-300 pb-3">
              <p className="text-[10px] uppercase font-bold text-slate-500">Republic of the Philippines</p>
              <h3 className="text-base font-black font-display text-slate-900">MUNICIPAL CEMETERY & BURIAL SERVICES</h3>
              <p className="text-[11px] text-purple-700 font-bold">OFFICIAL PERMIT OF INTERMENT</p>
            </div>

            <div className="text-left space-y-2 bg-slate-50 p-4 rounded-xl border border-slate-200">
              <p><span className="font-bold text-slate-500">Permit Number:</span> <strong className="font-mono text-purple-700">{selectedBurial.permit_no}</strong></p>
