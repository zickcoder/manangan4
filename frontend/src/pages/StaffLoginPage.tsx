...
type AuthStep = 'credentials' | 'otp' | 'verify_pin';
...
export function StaffLoginPage() {
  ...
  const [infoMsg, setInfoMsg] = useState(
    isTimeout ? '🔒 Session timed out after 30 minutes of inactivity. Please sign in again.' : ''
  );
  ...
  const sendEmailOtp = async (targetEmail: string, targetName: string, code: string) => {
    const templateParams = {
      ...
      to_name: targetName || 'Staff Officer',
      email: targetEmail,
      name: targetName || 'Staff Officer',
      user_email: targetEmail,
      user_name: targetName || 'Staff Officer',
      otp_code: code,
      otp: code,
      code: code,
      passcode: code,
      ...
      number: code,
      message: `Your GovServe Staff Portal login verification OTP code is ${code}.`,
      body: `Your GovServe Staff Portal login verification OTP code is ${code}.`,
    };
  ...
  const handleCredentialsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.user) {
        ...
        const isSameDay = isSameSessionSameDay(cleanEmail, 'staff', user);
        ...
        if (isSameDay) {
          ...
          await triggerOtpStep(user, '📩 New session detected. A 6-digit OTP code has been sent to your official email.');
        }
      } else {
  ...
  const triggerOtpStep = async (user: any, notice: string) => {
    ...
    const cleanEmail = (user.email || email).toLowerCase().trim();
    await sendEmailOtp(cleanEmail, user.name || 'Staff Officer', code);
    setSendingOtp(false);
    setAuthStep('otp');
    setInfoMsg(notice);
  };
  ...
  const handleVerifyOtp = (e: React.FormEvent) => {
    ...
    markOtpVerified(cleanEmail, 'staff');
  ...
  /** STEP 3: Verify 6-Digit PIN (max 5 attempts) */
  const handleVerifyPinSubmit = (e?: React.FormEvent) => {
    ...
    if (matches) {
      ...
      if (nextAttempts >= 5) {
        ...
        setPendingUser(null);
        setPassword('');
        setError('🔒 Maximum 5 PIN attempts reached. Please log in again.');
      } else {
  ...
  const finalizeLogin = (userToLogin: any, activePin?: string) => {
    ...
    sessionStorage.setItem('govserve_portal', 'staff');
    sessionStorage.setItem('govserve_staff_user', JSON.stringify(userWithPin));
    sessionStorage.setItem('govserve_user', JSON.stringify(userWithPin));
    localStorage.setItem('govserve_staff_user', JSON.stringify(userWithPin));
    
    // Set last active timestamp for 30-min timeout
    localStorage.setItem('govserve_last_active_staff', String(Date.now()));
  ...
  const handleSendForgotOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const code = Math.floor(100000 + Math.random() * 900000).toString();
      await sendEmailOtp(cleanEmail, 'Staff Officer', code);
      ...
      <div className="lg:w-1/2 bg-[#0B1E3D] text-white p-8 sm:p-12 lg:p-16 flex flex-col justify-between relative overflow-hidden min-h-[340px] lg:min-h-screen">
        ...
        <div className="relative z-10 my-auto py-8 lg:py-12 space-y-4 max-w-xl text-center mx-auto">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold font-display tracking-tight text-white leading-tight">
            Staff & Official Access
          </h1>
          <p className="text-xs sm:text-sm text-slate-300 leading-relaxed font-normal max-w-lg mx-auto">
            Authorized municipal officers, department heads, and system administrators only. All sessions are cryptographically logged and timed out after 30 minutes of inactivity.
          </p>
        </div>
            ...
            {!isForgotMode ? (
              <>
                ...
                {authStep === 'credentials' && (
                  <>
                    <div>
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-100 border border-slate-200 text-slate-700 text-[11px] font-bold mb-2">
                    ...
                    <form onSubmit={handleCredentialsSubmit} className="space-y-4">
                      ...
                      <button
                        ...
                        className="w-full py-3 px-4 rounded-xl bg-[#0B1E3D] hover:bg-[#15325b] text-white font-bold text-xs shadow-md shadow-slate-900/20 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 mt-2"
                      >
                        {lockoutSeconds > 0 ? (
                ...
                {/* ── STEP 2: OTP VERIFICATION (NEW DAY / NEXT SESSION) ── */}
                {authStep === 'otp' && (
                  <form onSubmit={handleVerifyOtp} className="space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-600 mt-1 leading-relaxed">
                        A 6-digit security code was dispatched to <strong className="text-blue-700 font-bold">{email}</strong>.
                      </p>
                    </div>
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      <button
                        ...
                        className="w-2/3 py-2.5 px-4 rounded-xl bg-[#0B1E3D] hover:bg-[#15325b] text-white font-bold text-xs shadow-md shadow-slate-900/20 transition-all cursor-pointer disabled:opacity-50"
                      >
                        Verify OTP →
                      </button>
                    </div>
                ...
                {authStep === 'verify_pin' && (
                  <form onSubmit={handleVerifyPinSubmit} className="space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                        Enter your 6-digit PIN to authenticate administrative access.
                      </p>
                    </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                    {forgotStep === 1 
                      ? 'Enter your official staff email address to receive a password reset verification code.'
                      : forgotStep === 2
                      ? `We dispatched a 6-digit code to ${forgotEmail}.`
                ...
                {forgotStep === 1 && (
                  <form onSubmit={handleSendForgotOtp} className="space-y-4">
                    ...
                    <button
                      ...
                      className="w-full py-3 px-4 rounded-xl bg-[#0B1E3D] hover:bg-[#15325b] text-white font-bold text-xs shadow-md shadow-slate-900/20 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                    >
                      {forgotSending ? (
                        <>
                          <Loader2 className="w-4 h-4 animate-spin" />
                ...
                {forgotStep === 2 && (
                  <form onSubmit={handleVerifyForgotOtp} className="space-y-4">
                    ...
                    <button
                      type="submit"
                      className="w-full py-3 px-4 rounded-xl bg-[#0B1E3D] hover:bg-[#15325b] text-white font-bold text-xs shadow-md shadow-slate-900/20 transition-all cursor-pointer"
                    >
                      Verify OTP Code →
                    </button>
                  </form>
